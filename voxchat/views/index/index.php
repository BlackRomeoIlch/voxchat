...
$rgba = function(string $hex, int $opacity): string {
    ...
    return 'rgba(' . $r . ',' . $g . ',' . $b . ',' . round($opacity / 100, 2) . ')';
};

// Gleiche uniqid-Logik wie Box
$uid         = 'page_' . substr(md5(microtime(true)), 0, 8);
...
<div id="irc-wrap-<?= $uid ?>">
            ...
            <form id="irc-form-<?= $uid ?>" autocomplete="off">
                ...
                <div class="input-group">
                    ...
                    <button class="btn btn-primary" type="submit">
                        <i class="fa-solid fa-paper-plane"></i> <?= $this->getTrans('send') ?>
                    </button>
                </div>
    ...
    <?php elseif (!$loggedIn) : ?>
        <div class="irc-input-area text-center">
            <a href="<?= $this->getUrl('user/login/index') ?>" class="btn btn-outline-secondary btn-sm">
                <?= $this->getTrans('loginToChat') ?>
            </a>
        </div>
    <?php else : ?>
        <div class="irc-input-area text-center">
            <span class="text-muted"><?= $this->getTrans('noWriteAccess') ?></span>
        </div>
    <?php endif; ?>
</div>
...
(function() {
    ...
    // lastId und renderedIds aus DOM initialisieren
    var renderedIds = {};
    var lastId = 0;
    ...
    function appendMessages(messages) {
        ...
        $.each(messages, function(i, msg) {
            ...
            if (isEmpty) { $messages.empty(); isEmpty = false; }
            ...
            $messages.append(
                '<div class="irc-msg" data-id="' + msg.id + '">'
    ...
    $('#irc-form-' + uid).on('submit', function(e) {
        ...
        $.ajax({
            type: 'POST', url: saveUrl, data: $form.serialize(), dataType: 'json',
            ...
            error: function() { alert(<?= json_encode($this->getTrans('saveFailed')) ?>); },
            complete: function() { $btn.prop('disabled', false); }
        });
    });

    $messages.on('scroll', function() {
        if (isAtBottom()) { newCount = 0; $badge.hide(); }
    });

    window['ircChat' + uid].scrollToBottom();
    if (refresh > 0) { setInterval(poll, refresh * 1000); }
}());
</script>
